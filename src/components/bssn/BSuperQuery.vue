            ...
            <a-row type="flex" style="margin-bottom:10px" :gutter="16" v-for="(item, index) in queryParamsModel" :key="index">
              ...
              <a-col :md="8" :xs="24" style="margin-bottom: 12px;">
                <template v-if="item.dictCode">
                  ...
                  <template v-else>
                    <b-multi-select-tag v-show="allowMultiple(item)" v-model="item.val" :dictCode="item.dictCode" placeholder="请选择"/>
                    <b-dict-select_tag v-show="!allowMultiple(item)" v-model="item.val" :dictCode="item.dictCode" placeholder="请选择"/>
                  </template>
                </template>
                <b-popup v-else-if="item.type === 'popup'" :value="item.val" v-bind="item.popup" group-id="superQuery" @input="(e,v)=>handleChangeJPopup(item,e,v)"/>
                <b-select-multi-user
                ...
                />
                <b-date v-else-if=" item.type=='date' " v-model="item.val" placeholder="请选择日期" style="width: 100%"></b-date>
                <b-date v-else-if=" item.type=='datetime' " v-model="item.val" placeholder="请选择时间" :show-time="true" date-format="YYYY-MM-DD HH:mm:ss" style="width: 100%"></b-date>
...
<script>
  ...
  import BDate from '@/components/bssn/BDate.vue'
  ...
  export default {
    name: 'BSuperQuery',
    mixins: [mixinDevice],
    components: {  BMultiSelectTag, BDate, BSelectMultiUser },
    props: {
      /*
       fieldList: [{
          value:'',
    ...
    data() {
      return {
        ...
        saveTreeData: [],
        // 保存查询条件的前缀名
        saveCodeBefore: 'BSuperQuerySaved_',
        // 查询类型，过滤条件匹配（and、or）
        matchType: 'and',
