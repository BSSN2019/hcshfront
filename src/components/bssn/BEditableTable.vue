...
<template>
  <a-spin :spinning="loading">
    ...
    <div :id="`${caseId}inputTable`" class="input-table">
                    ...
                    <template v-else-if="col.type === formTypes.date || col.type === formTypes.datetime">
                      ...
                      <span
                        ...
                        <b-date
                          ...
                          allowClear
                          @change="(v)=>handleChangeBDateCommon(v,id,row,col,col.type === formTypes.datetime)"/>
                      ...
                      <a-tooltip
                        ...
                        <span
                          ...
                          <b-input-pop
                            ...
                            :getCalendarContainer="getParentContainer"
                            :placeholder="replaceProps(col, col.placeholder)"
                            @change="(v)=>handleChangeBInputPopCommon(v,id,row,col)">
                          </b-input-pop>
                        </span>
                      </a-tooltip>
                    </template>
      ...
      <b-file-pop ref="filePop" @ok="handleFileSuccess"></b-file-pop>
    </div>
  </a-spin>
</template>
...
<script>
  ...
  import { FormTypes, VALIDATE_NO_PASSED } from '@/utils/BEditableTableUtil'
  import { cloneObject, randomString, randomNumber } from '@/utils/util'
  import BDate from '@/components/bssn/BDate'
  import { initDictOptions } from '@/components/dict/BDictSelectUtil'
  import { getFileAccessHttpUrl } from '@/api/manage';
  import BInputPop from '@/components/bssn/minipop/BInputPop'
  import BFilePop from '@/components/bssn/minipop/BFilePop'
  ...
  export default {
    name: 'BEditableTable',
    components: { BDate, Draggable, BInputPop, BFilePop },
    provide() {
      return {
        parentIsBEditableTable: true,
    ...
    methods: {
      ...
      removeRows(id) {
        ...
        if (!(id instanceof Array)) {
          if (typeof id === 'string') {
            ...
            throw  `BEditableTable.removeRows() 函数需要的参数可以是string或Array类型，但提供的却是${typeof id}`
          }
        }
      ...
      handleChangeBDateCommon(value, id, row, column, showTime) {
      ...
      handleChangeBInputPopCommon(value, id, row, column){
      ...
      _loadDictConcatToOptions(column) {
        initDictOptions(column.dictCode).then((res) => {
          if (res.success) {
            ...
            console.group(`BEditableTable 查询字典(${column.dictCode})发生异常`)
          ...
          Object.keys(others).map((key) => {
            this.columns.map(k=>{
              if(k.key === key){
                ...
                if(k.type === 'date'){
                  this.handleChangeBDateCommon(others[key], key+tempId, {id:tempId}, k, false)
                }else if(k.type === 'datetime'){
                  this.handleChangeBDateCommon(others[key], key+tempId, {id:tempId}, k, true)
                }else{
